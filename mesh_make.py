...
# === 1. 载入 STEP 文件 ===
step_path = "gear_step//assembled_gear_pair.step"
...
# === 3. 改进的表面物理组定义 ===
def find_hole_surfaces(gear_volume_tag, origin_point, max_radial_distance=0.3, angle_tol=30):
    """
    精确识别内径面（孔的内表面）
...
print("识别孔表面...")
gear1_hole = find_hole_surfaces(gear1_tag, (0,0,0))
print(f"齿轮1孔表面: {len(gear1_hole)}个面")
gear2_hole = find_hole_surfaces(gear2_tag, (73.126,0,0))
...
# === 8. 生成 .sur 文件 ===
sur_file = "assembled_gears_curvature.sur"
try:
    with open(sur_file, "w") as f:
        for dim, pg in gmsh.model.getPhysicalGroups(dim=2):
            try:
                ...
                continue
    print(f"已生成 SURFACE 文件: {sur_file}")
except Exception as e:
    print(f"生成SUR文件失败: {str(e)}")
